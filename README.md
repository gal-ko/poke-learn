...
No build step, no dependencies, no server required. Just open `index.html` in any modern browser:

```
git clone https://github.com/gal-ko/poke-learn.git
open poke-learn/index.html
```
...
- **Offline-ready** — runs entirely client-side with no network calls (except optional Google Fonts for the dark layout)
...
└── src/
    ...
    ├── config/             # Static configuration (themes, layouts, screens, cards, difficulty)
    ├── data/               # Domain data (Pokemon list, types, colors, sprite metadata)
    ...
    ├── fonts/              # Self-hosted Fredoka WOFF2 font files
...
Plain HTML, CSS, and vanilla JavaScript. No frameworks, bundlers, or transpilers. State is persisted in `localStorage`. Sprites are served from a pre-built sprite sheet generated by `tools/build-sprite-sheet.py`.
